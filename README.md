...
- help 与错误后的可用用法会显示简短参数类型；可选参数使用 `[count:integer]`，必填参数使用 `<name:text>`
- 枚举值的 `|` 分隔符使用独立颜色，便于快速分辨候选值
- 根命令没有显式执行器时，直接执行根命令会自动显示帮助
- 同一 Bukkit 命令匹配多棵命令树时，根帮助会合并所有 endpoint
- 语法错误后的候选按已匹配深度筛选并限制数量，避免无关分支淹没有用提示
- `[]` 只表示真正的可选参数，不用于包装普通字面量子命令
- 可注入 i18n 文案函数
...
如果不注入，框架会使用内置英文默认文案。
自定义文案函数缺少某个框架 key 并原样返回该 key 时，框架会回退到对应的内置英文文案，避免把 key 直接发送给玩家。
...
- `command.help.header`：参数 `root`
- `command.help.empty`
- `command.help.endpoint`：参数 `root`、`path`、`params`、`description`、`permission`
